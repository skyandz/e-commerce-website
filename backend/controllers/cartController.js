...
export async function updateCf(req,res) {
    console.log(`PUT /carts/${req.params.id} is requested`)
    try {
        const cf = await database.query({
            text:`UPDATE "public"."carts" SET "cartCf"='true' WHERE "cartId"= $1`,
...
export async function deleteCart(req,res){
    console.log(`DELETE /carts/${req.params.id} is requested`)
    try {
            const delete_cartDtl = await database.query({
                text:`DELETE FROM "cartDtl" WHERE "cartId" = $1`,
                values:[req.params.id]
            })
            const delete_carts = await database.query({
